...
const RAW: SettingMeta[] = [
  ...
  },
  {
    ...
    whenToChange: "When switching between Anthropic and OpenAI (e.g. cost, quality, or contractual reasons).",
    ifWrong: "If the matching API key isn't set for the new provider, all AI features will fail until a working key is provided.",
    tags: ["ai", "provider", "anthropic", "openai", "llm"],
    dangerLevel: "high",
    domain: "system",
    type: "select",
    selectOptions: [
  ...
  },
  {
    key: "ai_api_key",
    label: "AI API key",
    description: "API key for the selected provider. Stored encrypted; never displayed after saving.",
    whenToChange: "When rotating keys, switching providers, or configuring AI for the first time.",
    ifWrong: "All AI features will fail. A wrong key may also rack up failed-call costs depending on the provider's billing.",
    tags: ["ai", "api", "key", "secret", "credential"],
    dangerLevel: "high",
    domain: "system",
    type: "password",
